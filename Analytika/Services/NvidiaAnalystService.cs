...
public class NvidiaAnalystService : INvidiaAnalystService
{
    ...
    public async Task<AiAnalystResult> AskAsync(string question, string? userId, string? userName, CancellationToken ct = default)
    {
        ...
        if (string.IsNullOrWhiteSpace(apiKey))
            return Fail(result, "No NVIDIA API key is configured. Add one in Admin → AI Agent.");
    ...
    public async Task AskStreamAsync(string question, string? userId, string? userName, Func<object, Task> emit, CancellationToken ct = default)
    {
        ...
        try
        {
            ...
            var apiKey = await _settingsService.GetApiKeyAsync();
            if (string.IsNullOrWhiteSpace(apiKey)) { await Emit(new { error = "No NVIDIA API key is configured (Admin → AI Agent)." }); return; }
    ...
    public async Task<(bool ok, string message)> TestConnectionAsync(CancellationToken ct = default)
    {
        var settings = await _settingsService.GetAsync();
        var apiKey = await _settingsService.GetApiKeyAsync();
        if (string.IsNullOrWhiteSpace(apiKey)) return (false, "No API key configured.");
        try
        {
        ...
        AiSettings settings, string apiKey, string system, string user, CancellationToken ct)
    {
        ...
        if (!resp.IsSuccessStatusCode)
            throw new InvalidOperationException($"NVIDIA API returned {(int)resp.StatusCode}: {Truncate(body, 300)}");
    ...
    {
        var endpoint = $"{(settings.ApiBaseUrl ?? "").TrimEnd('/')}/chat/completions";
        var payload = new
        {
            model = settings.Model,
        ...
        if (!resp.IsSuccessStatusCode)
        {
            ...
            throw new InvalidOperationException($"NVIDIA API returned {(int)resp.StatusCode}: {Truncate(err, 300)}");
        }
