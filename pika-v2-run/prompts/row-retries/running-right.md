Create Codex pet row `running-right` for `pika`: exactly 8 full-body frames in one horizontal strip on flat pure cyan #00FFFF.

Use the attached canonical base for identity and the layout guide only for spacing. Same pet in every frame: Preserve the existing 8-bit pixel Pika: compact yellow electric mouse, red cheek marks, black-tipped ears, lightning-bolt tail, black pixel outlines, transparent background. Upgrade only with coherent looking directions.. Preserve silhouette, face, palette, material, proportions, markings, and props.
